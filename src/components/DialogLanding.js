...
const DialogLanding = ({ visible, setVisible, choose, setchoose }) => {
    ...
    const hideDialog = () => (setVisible(false), setshowValidation({}), setfileName({}))
        ...
        if (filtertrue.length == 0 && filtervalidation?.length == 0) {
            ...
            if (Object.keys(answers).length > 1 || Object.keys(value).length) {
                ...
                try {
                    ...
                    hideDialog()
                    setchoose({ ...choose, members: [...choose.members, result?.memberId] })
                        , setanswers({ ans: '' }), setValue({}), setfileName({})
                } catch (error) {
                    console.log(error)
                    hideDialog()
                }
    ...
    return (
        <Portal>
            ...
            <Dialog dismissable={false} style={{ flex: 1, height: height, backgroundColor: '#F0EFEA', width: '95%', alignSelf: 'center' }} visible={visible} onDismiss={() => hideDialog()}>
                {show && (
                    <DateTimePicker
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingVertical: 10 }} >
                        ...
                        </View>
                        {
                            choose.attachmentUrl ?
                                <TouchableOpacity onPress={() => checkPermission(choose?.attachmentUrl)}
                                    style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <Text style={{ marginRight: 52 }}>Attachment</Text>
                                    <FontAwesome
                                        name="download"
                                        size={14}
                                        color={'black'}
                        ...
                        <View >
                            {
                                eachDataAc?.map((item, i) => (
                                    <View key={i + 1} style={{
                                        elevation: 5, padding: 5, marginVertical: 8, borderRadius: 10, backgroundColor: '#FFFFFF', width: '100%'
                                    }} >
                                            ...
                                            <View>
                                                ...
                                                </View>
                                                {
                                                    (item.attachmentUrl || item.attechmentUrl) ?
                                                        <TouchableOpacity onPress={() => checkPermission(item.attechmentUrl || item.attachmentUrl)} style={{ flexDirection: 'row', alignItems: 'center', marginVertical: 6 }}>
                                                            <Text style={{ marginRight: 52 }}>Attachment</Text>
                                                            <FontAwesome
                                                                name="download"
                                                                size={14}
